#!/bin/bash
script_path="$HOME/sample-peppapigdetection/script"
model_path="$HOME/sample-peppapigdetection/MyModel"
function download()
{
    model_name=$1
    License_OK=$2
...
main()
{
    echo "Peppapigdetectionapp prepareing"

    C31_flag=`find $HOME -maxdepth 1 -name "sample-peppapigdetection" 2> /dev/null`
    if [[ $C31_flag ]];then
        read -p "[INFO] The sample-peppapigdetection is existence.Do you want to re-prepare ? [Y/N]: " response
        if [ $response"z" = "Nz" ] || [ $response"z" = "nz" ]; then
            echo "Exit prepareing"
            return 1
        elif [ $response"z" = "Yz" ] || [ $response"z" = "yz" ] || [ $response"z" = "z" ]; then
            echo "[INFO] Please manually delete $HOME/sample-peppapigdetection director and re-execute this script"
            return 1
        else
    ...
    mkdir $HOME/sample-peppapigdetection
    if [[ $? -ne 0 ]];then
    ...
    git clone https://gitee.com/Atlas200DK/sample-peppapigdetection.git $HOME/sample-peppapigdetection
    if [[ $? -ne 0 ]];then
