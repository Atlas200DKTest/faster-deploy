#!/bin/bash
script_path="$HOME/sample-colorization/script"
model_path="$HOME/sample-colorization/MyModel"
function download()
{
    model_name=$1
    License_OK=$2
...
main()
{
    echo "Colorizationapp prepareing"

    C31_flag=`find $HOME -maxdepth 1 -name "sample-colorization" 2> /dev/null`
    if [[ $C31_flag ]];then
        read -p "[INFO] The sample-colorization is existence.Do you want to re-prepare ? [Y/N]: " response
        if [ $response"z" = "Nz" ] || [ $response"z" = "nz" ]; then
            echo "Exit prepareing"
            return 1
        elif [ $response"z" = "Yz" ] || [ $response"z" = "yz" ] || [ $response"z" = "z" ]; then
            echo "[INFO] Please manually delete $HOME/sample-colorization director and re-execute this script"
            return 1
        else
    ...
    mkdir $HOME/sample-colorization
    if [[ $? -ne 0 ]];then
    ...
    git clone https://gitee.com/Atlas200DK/sample-colorization.git $HOME/sample-colorization
    if [[ $? -ne 0 ]];then
