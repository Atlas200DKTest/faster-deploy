#!/bin/bash
script_path="$HOME/sample-headposeestimationpython/model"
model_path="$HOME/sample-headposeestimationpython/MyModel"
function download()
{
    model_name=$1
    License_OK=$2
...
main()
{
    ...
    C31_flag=`find $HOME -maxdepth 1 -name "sample-headposeestimationpython" 2> /dev/null`
    if [[ $C31_flag ]];then
        ...
        if [ $response"z" = "Nz" ] || [ $response"z" = "nz" ]; then
            ...
            echo "[INFO] Please manually delete $HOME/sample-headposeestimationpython director and re-execute this script"
            return 1
        else
    ...
    mkdir $HOME/sample-headposeestimationpython
    if [[ $? -ne 0 ]];then
    ...
    git clone https://gitee.com/Atlas200DK/sample-headposeestimation-python.git $HOME/sample-headposeestimationpython
    if [[ $? -ne 0 ]];then
