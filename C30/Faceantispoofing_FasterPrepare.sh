#!/bin/bash
script_path="$HOME/sample-faceantispoofing/script"
model_path="$HOME/sample-faceantispoofing/MyModel"
function download()
{
    model_name=$1
    License_OK=$2
...
main()
{
    ...
    if [[ $C31_flag ]];then
        ...
        if [ $response"z" = "Nz" ] || [ $response"z" = "nz" ]; then
            ...
            echo "[INFO] Please manually delete $HOME/sample-faceantispoofing director and re-execute this script"
            return 1
        else
