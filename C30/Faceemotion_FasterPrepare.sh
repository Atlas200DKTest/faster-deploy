#!/bin/bash
script_path="$HOME/sample-faceemotion/script"
model_path="$HOME/sample-faceemotion/MyModel"
function download()
{
    model_name=$1
    License_OK=$2
...
main()
{
    echo "Faceemotionapp prepareing"

    C31_flag=`find $HOME -maxdepth 1 -name "sample-faceemotion" 2> /dev/null`
    if [[ $C31_flag ]];then
        read -p "[INFO] The sample-faceemotion is existence.Do you want to re-prepare ? [Y/N]: " response
        if [ $response"z" = "Nz" ] || [ $response"z" = "nz" ]; then
            echo "Exit prepareing"
            return 1
        elif [ $response"z" = "Yz" ] || [ $response"z" = "yz" ] || [ $response"z" = "z" ]; then
            echo "[INFO] Please manually delete $HOME/sample-faceemotion director and re-execute this script"
            return 1
        else
    ...
    git clone https://gitee.com/Atlas200DK/sample-faceemotion.git $HOME/sample-faceemotion
    if [[ $? -ne 0 ]];then
