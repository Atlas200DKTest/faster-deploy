#!/bin/bash
model_path="$HOME/sample-segmentation/MyModel"
function download()
{
    model_name=$1
    License_OK=$2
...
main()
{
    ...
    C31_flag=`find $HOME -maxdepth 1 -name "sample-segmentation" 2> /dev/null`
    if [[ $C31_flag ]];then
        ...
        if [ $response"z" = "Nz" ] || [ $response"z" = "nz" ]; then
            ...
            echo "[INFO] Please manually delete $HOME/sample-segmentation director and re-execute this script"
            return 1
        else
    ...
    mkdir $HOME/sample-segmentation
    if [[ $? -ne 0 ]];then
    ...
    git clone https://gitee.com/Atlas200DK/sample-segmentation.git $HOME/sample-segmentation
    if [[ $? -ne 0 ]];then
