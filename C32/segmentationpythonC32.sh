#!/bin/bash
script_path="$HOME/AscendProjects/sample-segmentation-python/segmentationapp/models"
...
main()
{
    echo "Deploying segmentationpythonapp..."

    C31_flag=`find $HOME/AscendProjects -maxdepth 1 -name "sample-segmentation-python" 2> /dev/null`
    if [[ $C31_flag ]];then
        read -p "[INFO] The sample-segmentation-python already exists. Do you want to replace it? [Y/N]:" response
        if [ $response"z" = "Nz" ] || [ $response"z" = "nz" ]; then
            echo "Exit prepareing"
            return 1
        elif [ $response"z" = "Yz" ] || [ $response"z" = "yz" ] || [ $response"z" = "z" ]; then
            rm -rf $HOME/AscendProjects/sample-segmentation-python
            if [[ $? -ne 0 ]];then
                echo "[ERROR] Execute rm command failed. Please manually delete $HOME/AscendProjects/sample-segmentation-python director and re-execute this script."
                return 1
            fi
        else
    ...
    mkdir $HOME/AscendProjects/sample-segmentation-python
    if [[ $? -ne 0 ]];then
        echo "[ERROR] Execute mkdir command failed. Please make sure you have the right permission."
        return 1
    fi

    git clone https://gitee.com/Atlas200DK/sample-segmentation-python.git $HOME/AscendProjects/sample-segmentation-python
    if [[ $? -ne 0 ]];then
