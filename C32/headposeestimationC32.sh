#!/bin/bash
script_path="$HOME/AscendProjects/sample-headposeestimation/script"
model_path="$HOME/AscendProjects/sample-headposeestimation/MyModel"
...
main()
{
    echo "Deploying headposeestimationapp..."

    C31_flag=`find $HOME/AscendProjects -maxdepth 1 -name "sample-headposeestimation" 2> /dev/null`
    if [[ $C31_flag ]];then
        read -p "[INFO] The sample-headposeestimation already exists. Do you want to replace it? [Y/N]:" response
        if [ $response"z" = "Nz" ] || [ $response"z" = "nz" ]; then
            echo "Exit prepareing"
            return 1
        elif [ $response"z" = "Yz" ] || [ $response"z" = "yz" ] || [ $response"z" = "z" ]; then
            rm -rf $HOME/AscendProjects/sample-headposeestimation
            if [[ $? -ne 0 ]];then
                echo "[ERROR] Execute rm command failed. Please manually delete $HOME/AscendProjects/sample-headposeestimation director and re-execute this script."
                return 1
            fi
        else
    ...
    mkdir $HOME/AscendProjects/sample-headposeestimation
    if [[ $? -ne 0 ]];then
    ...
    echo "[INFO] Please follow the link from the 编译 step in Readme to continue running the case.(https://gitee.com/Atlas200DK/sample-headposeestimation)"
}
main
