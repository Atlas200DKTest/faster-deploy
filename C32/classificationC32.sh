...
main()
{
    echo "Deploying classificationapp..."

    C31_flag=`find $HOME/AscendProjects -maxdepth 1 -name "sample-classification" 2> /dev/null`
    if [[ $C31_flag ]];then
        read -p "[INFO] The sample-classification already exists. Do you want to replace it? [Y/N]:" response
        if [ $response"z" = "Nz" ] || [ $response"z" = "nz" ]; then
            echo "Exit prepareing"
            return 1
        elif [ $response"z" = "Yz" ] || [ $response"z" = "yz" ] || [ $response"z" = "z" ]; then
            rm -rf $HOME/AscendProjects/sample-classification
            if [[ $? -ne 0 ]];then
                echo "[ERROR] Execute rm command failed. Please manually delete $HOME/AscendProjects/sample-classification director and re-execute this script."
                return 1
            fi
        else
    ...
    echo "[INFO] Please follow the link from the 编译 step in Readme to continue running the case.(https://gitee.com/Atlas200DK/sample-classification/tree/1.3x.0.0/)"
}
main
