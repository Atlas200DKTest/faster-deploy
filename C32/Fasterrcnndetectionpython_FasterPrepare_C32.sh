#!/bin/bash
script_path="$HOME/AscendProjects/sample-fasterrcnndetectionpython/model"
model_path="$HOME/AscendProjects/sample-fasterrcnndetectionpython/MyModel"
function download()
{
    model_name=$1
    License_OK=$2
...
main()
{
    ...
    C31_flag=`find $HOME/AscendProjects -maxdepth 1 -name "sample-fasterrcnndetectionpython" 2> /dev/null`
    if [[ $C31_flag ]];then
        ...
        if [ $response"z" = "Nz" ] || [ $response"z" = "nz" ]; then
            ...
            echo "[INFO] Please manually delete $HOME/AscendProjects/sample-fasterrcnndetectionpython director and re-execute this script"
            return 1
        else
    ...
    mkdir $HOME/AscendProjects/sample-fasterrcnndetectionpython
    if [[ $? -ne 0 ]];then
        echo "[ERROR] Execute mkdir command failed, Please check your environment"
        return 1
    fi  
    ...
    git clone https://gitee.com/Atlas200DK/sample-fasterrcnndetection-python.git $HOME/AscendProjects/sample-fasterrcnndetectionpython --branch 1.3x.0.0
    if [[ $? -ne 0 ]];then
