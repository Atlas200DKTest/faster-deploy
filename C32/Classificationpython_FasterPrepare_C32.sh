#!/bin/bash
script_path="$HOME/AscendProjects/sample-classificationpython/classificationapp/models"
model_path="$HOME/AscendProjects/sample-classificationpython/MyModel"
function download()
{
    model_name=$1
    License_OK=$2
...
main()
{
    ...
    C31_flag=`find $HOME/AscendProjects -maxdepth 1 -name "sample-classificationpython" 2> /dev/null`
    if [[ $C31_flag ]];then
        read -p "[INFO] The sample-classificationpython is existence.Do you want to re-prepare ? [Y/N]: " response
        if [ $response"z" = "Nz" ] || [ $response"z" = "nz" ]; then
            echo "Exit prepareing"
            return 1
        elif [ $response"z" = "Yz" ] || [ $response"z" = "yz" ] || [ $response"z" = "z" ]; then
            echo "[INFO] Please manually delete $HOME/AscendProjects/sample-classificationpython director and re-execute this script"
            return 1
        else
    ...
    mkdir $HOME/AscendProjects/sample-classificationpython
    if [[ $? -ne 0 ]];then
        echo "[ERROR] Execute mkdir command failed, Please check your environment"
        return 1
    fi  
    ...
    git clone https://gitee.com/Atlas200DK/sample-classification-python.git $HOME/AscendProjects/sample-classificationpython --branch 1.3x.0.0
    if [[ $? -ne 0 ]];then
