...
main()
{
    echo "classificationpythonapp prepareing"

    C31_flag=`find $HOME/AscendProjects -maxdepth 1 -name "sample-classification-python" 2> /dev/null`
    if [[ $C31_flag ]];then
        read -p "[INFO] The sample-classification-python is existence.Do you want to re-prepare ? [Y/N]: " response
        if [ $response"z" = "Nz" ] || [ $response"z" = "nz" ]; then
            echo "Exit prepareing"
            return 1
        elif [ $response"z" = "Yz" ] || [ $response"z" = "yz" ] || [ $response"z" = "z" ]; then
            echo "[INFO] Please manually delete $HOME/AscendProjects/sample-classification-python director and re-execute this script"
            return 1
        else
    ...
    echo "https://gitee.com/Atlas200DK/sample-classification-python"
}
main
