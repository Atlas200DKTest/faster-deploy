#!/bin/bash
script_path="$HOME/AscendProjects/sample-classification-python/classificationapp/models"
model_path="$HOME/AscendProjects/sample-classification-python/MyModel"
...
main()
{
    echo "Deploying classificationpythonapp..."

    C31_flag=`find $HOME/AscendProjects -maxdepth 1 -name "sample-classification-python" 2> /dev/null`
    if [[ $C31_flag ]];then
        read -p "[INFO] The sample-classification-python already exists. Do you want to replace it? [Y/N]:" response
        if [ $response"z" = "Nz" ] || [ $response"z" = "nz" ]; then
            echo "Exit prepareing"
            return 1
        elif [ $response"z" = "Yz" ] || [ $response"z" = "yz" ] || [ $response"z" = "z" ]; then
            rm -rf $HOME/AscendProjects/sample-classification-python
            if [[ $? -ne 0 ]];then
                echo "[ERROR] Execute rm command failed. Please manually delete $HOME/AscendProjects/sample-classification-python director and re-execute this script."
                return 1
            fi
        else
    ...
    mkdir $HOME/AscendProjects/sample-classification-python
    if [[ $? -ne 0 ]];then
        echo "[ERROR] Execute mkdir command failed. Please make sure you have the right permission."
        return 1
    fi

    git clone https://gitee.com/Atlas200DK/sample-classification-python.git $HOME/AscendProjects/sample-classification-python
    if [[ $? -ne 0 ]];then
