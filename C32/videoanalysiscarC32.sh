...
script_path="$HOME/AscendProjects/sample-videoanalysiscar/script"
model_path="$HOME/AscendProjects/sample-videoanalysiscar/MyModel"
...
main()
{
    echo "Deploying videoanalysiscarapp..."

    C31_flag=`find $HOME/AscendProjects -maxdepth 1 -name "sample-videoanalysiscar" 2> /dev/null`
    if [[ $C31_flag ]];then
        read -p "[INFO] The sample-videoanalysiscar already exists. Do you want to replace it? [Y/N]:" response
        if [ $response"z" = "Nz" ] || [ $response"z" = "nz" ]; then
            echo "Exit prepareing"
            return 1
        elif [ $response"z" = "Yz" ] || [ $response"z" = "yz" ] || [ $response"z" = "z" ]; then
            rm -rf $HOME/AscendProjects/sample-videoanalysiscar
            if [[ $? -ne 0 ]];then
                echo "[ERROR] Execute rm command failed. Please manually delete $HOME/AscendProjects/sample-videoanalysiscar director and re-execute this script."
                return 1
            fi
        else
    ...
    mkdir $HOME/AscendProjects/sample-videoanalysiscar
    if [[ $? -ne 0 ]];then
        echo "[ERROR] Execute mkdir command failed. Please make sure you have the right permission."
        return 1
    fi

    git clone https://gitee.com/Atlas200DK/sample-videoanalysiscar.git $HOME/AscendProjects/sample-videoanalysiscar --branch 1.3x.0.0
    if [[ $? -ne 0 ]];then
    ...
    echo "[INFO] Please follow the link from the 编译 step in Readme to continue running the case.(https://gitee.com/Atlas200DK/sample-videoanalysiscar/tree/1.3x.0.0/)"
}
main
