#!/bin/bash
script_path="$HOME/AscendProjects/sample-headposeestimation-python/model"
model_path="$HOME/AscendProjects/sample-headposeestimation-python/MyModel"
...
main()
{
    echo "Deploying Headposeestimationpythonapp..."

    C31_flag=`find $HOME/AscendProjects -maxdepth 1 -name "sample-headposeestimation-python" 2> /dev/null`
    if [[ $C31_flag ]];then
        read -p "[INFO] The sample-headposeestimation-python already exists. Do you want to replace it? [Y/N]:" response
        if [ $response"z" = "Nz" ] || [ $response"z" = "nz" ]; then
            echo "Exit prepareing"
            return 1
        elif [ $response"z" = "Yz" ] || [ $response"z" = "yz" ] || [ $response"z" = "z" ]; then
            rm -rf $HOME/AscendProjects/sample-headposeestimation-python
            if [[ $? -ne 0 ]];then
                echo "[ERROR] Execute rm command failed. Please manually delete $HOME/AscendProjects/sample-headposeestimation-python director and re-execute this script."
                return 1
            fi
        else
    ...
    mkdir $HOME/AscendProjects/sample-headposeestimation-python
    if [[ $? -ne 0 ]];then
