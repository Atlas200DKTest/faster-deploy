#!/bin/bash
script_path="$HOME/AscendProjects/sample-faceantispoofing/script"
model_path="$HOME/AscendProjects/sample-faceantispoofing/MyModel"
...
function download()
{
	...
	if [ ${model_name} == "face_detection" ];then
	    ...
	    export SLOG_PRINT_TO_STDOUT=1 && export PATH=${PATH}:${DDK_HOME}/uihost/toolchains/ccec-linux/bin/ && export LD_LIBRARY_PATH=${DDK_HOME}/uihost/lib/ && export TVM_AICPU_LIBRARY_PATH=${DDK_HOME}/uihost/lib/:${DDK_HOME}/uihost/toolchains/ccec-linux/aicpu_lib && export TVM_AICPU_INCLUDE_PATH=${DDK_HOME}/include/inc/tensor_engine && export PYTHONPATH=${DDK_HOME}/site-packages && export TVM_AICPU_OS_SYSROOT=/usr/aarch64-linux-gnu && ${DDK_HOME}/uihost/bin/omg --output="${script_path}/depl" --check_report=${model_path}/${model_name}_result.json --plugin_path= --model="${model_path}/${model_name}.prototxt" --framework=0 --ddk_version=${tools_version} --weight="${model_path}/${model_name}.caffemodel" --input_shape=${model_shape}
	fi
	if [ $? -ne 0 ];then
...
main()
{
    echo "Deploying faceantispoofing..."

    C31_flag=`find $HOME/AscendProjects -maxdepth 1 -name "sample-faceantispoofing" 2> /dev/null`
    if [[ $C31_flag ]];then
        read -p "[INFO] The sample-faceantispoofing already exists. Do you want to replace it? [Y/N]:" response
        if [ $response"z" = "Nz" ] || [ $response"z" = "nz" ]; then
            echo "Exit prepareing"
            return 1
        elif [ $response"z" = "Yz" ] || [ $response"z" = "yz" ] || [ $response"z" = "z" ]; then
            rm -rf $HOME/AscendProjects/sample-faceantispoofing
            if [[ $? -ne 0 ]];then
                echo "[ERROR] Execute rm command failed. Please manually delete $HOME/AscendProjects/sample-faceantispoofing director and re-execute this script."
                return 1
            fi
        else
    ...
    mkdir $HOME/AscendProjects/sample-faceantispoofing
    if [[ $? -ne 0 ]];then
        echo "[ERROR] Execute mkdir command failed. Please make sure you have the right permission."
        return 1
    fi  

    git clone https://gitee.com/Atlas200DK/sample-faceantispoofing.git $HOME/AscendProjects/sample-faceantispoofing --branch 1.3x.0.0
    if [[ $? -ne 0 ]];then
    ...
    echo "[INFO] Please follow the link from the 编译 step in Readme to continue running the case.(https://gitee.com/Atlas200DK/sample-faceantispoofing/tree/1.3x.0.0/)"
}
main
